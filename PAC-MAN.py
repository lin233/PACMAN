...
def rebackLevel2():   #第二关失败后重来的窗口按钮调用的函数
    askFail2.destroy()
    paintCanvas2()

def rebackLevel3():   #第三关失败后重来的窗口按钮调用的函数
    askFail3.destroy()
    paintCanvas3()

def rebackLevel4():   #第四关失败后重来的窗口按钮调用的函数
    askFail4.destroy()
    paintCanvas4()

def rebackLevel5():   #第五关失败后重来的窗口按钮调用的函数
    askFail5.destroy()
    paintCanvas5()

def continueLevel2():   #第一关成功后继续的窗口按钮调用的函数，关闭该窗口并进行下一关
    ask.destroy()       #关闭
    paintCanvas2()      #重开

def continueLevel3():   #第二关成功后继续的窗口按钮调用的函数
    ask2.destroy()
    paintCanvas3()

def continueLevel4():   #第三关成功后继续的窗口按钮调用的函数
    ask3.destroy()
    paintCanvas4()

def continueLevel5():   #第四关成功后继续的窗口按钮调用的函数
    ask4.destroy()
    paintCanvas5()

def destroyI():   #无敌版胜利窗口的关闭
    askI.destroy()

def rebackI():   #无敌版失败窗口的关闭和重开
    askFailI.destroy()  #关闭
    Invincible()    #重开

def Right(i,L,cv):      #检验右边是否通路  ， 通路返回True ，不通返回 False。  #（i 是怪物的标签，L是关卡数，cv是画布，同下。）
                                #在第L关中检验怪物的右方是否为墙，为墙不通，没有墙则通 （Right,Left,Up,Down函数内部原理相同,就不多加赘述）
    walls=[9,16,16,13,26,9]   #每关中墙的最大图形项加一
    xyb=list(cv.coords(i))    #返回怪物i的坐标
    xyb_=list(cv.find_overlapping(xyb[0]-14+1,xyb[1]-14,xyb[0]+14+1,xyb[1]+14))  # 返回怪物右方一个像素的矩形范围内的图形项 
    if xyb_[0]<walls[L-1]:      #因为墙的图形项是最小的那几个，所以只要第一项不是墙，就没有墙了
        return False   #撞墙了 返回False
    return True   #没撞墙 返回True

def Left(i,L,cv):   #检验左边是否通路  ， 通路返回True ，不通返回 False #（i 是怪物的标签，L是关卡数，cv是画布，同下。）
...
def Up(i,L,cv):   #检验上边是否通路，通路返回True ，不通返回 False #（i 是怪物的标签，L是关卡数，cv是画布，同下。）
    walls=[9,16,16,13,26,9]
    xyb=list(cv.coords(i))
...
def Down(i,L,cv):   #检验下边是否通路，通路返回True，不通返回False #（i 是怪物的标签，L是关卡数，cv是画布，同下。）
    walls=[9,16,16,13,26,9]
    xyb=list(cv.coords(i))
    xyb_=list(cv.find_overlapping(xyb[0]-14,xyb[1]-14+1,xyb[0]+14,xyb[1]+14+1))
    if xyb_[0]<walls[L-1]:
...
def zhq(i,x,y,L,cv):   #在第L关中检验怪物是否撞墙并移动
                        #   i是怪物的标签 ， x,y是要前进的方向。x为正是向右，y为正向下。
    walls=[9,16,16,13,26,9]   #墙壁的最大图形项+1
    xyb = list(cv.coords(i))   #返回怪物i的中心坐标
    xyb_ = list(cv.find_overlapping(xyb[0] - 14 + x, xyb[1] - 14 + y, xyb[0] + 14 + x, xyb[1] + 14 + y)) #返回怪物前方一个像素的图形项并储存在列表中，以判断是否会撞墙
    if xyb_[0] < walls[L-1]:  #如果前方是墙
        cv.move(i, 0, 0)   #不移动
    else:                  #前方没有墙
        cv.move(i, x, y)   #移动


def zhqEater(x,y,i,L,cv):   #在第L关中检验吃豆人是否撞墙并移动，x,y是吃豆人要前进的方向，x是水平方向，向右为正，y是竖直方向，向下为正。i是吃豆人的标签，L是关卡数，cv是画布。
    walls=[9,16,16,13,26,9]    #墙壁的最大图形项+1
    xyc__=list(cv.coords(i))   #返回吃豆人的中心坐标
    xyc___=list(cv.find_overlapping(xyc__[0]-8+x,xyc__[1]-8+y,xyc__[0]+8+x,xyc__[1]+8+y))  #返回吃豆人的矩形范围前进的方向上的图形项，以判断是否会撞墙
    if xyc___[0]<walls[L-1]: #撞墙了
        cv.move(i, 0, 0)   #不移动
    else:               #没有撞墙
        cv.move(i, x, y)   #移动
        
#吃豆人和怪物的移动函数相似，但是没有合并，因为参考范围的问题，怪物需要用的矩形范围是28*28，这样可以使怪物不会卡在道路中，否则Up Down函数的判定会出问题
        #而吃豆人的参考范围只用了16*16，可以使吃豆人在转弯的时候的容错性更好，更容易转弯。

def budongle(i,x,y,L,cv):   #当怪物走进死路的时候需要掉头 ，i是怪物的标签，x，y是怪物前进的方向，x水平，向右为正，y竖直，向下为正
    back=0    # back是怪物四个方向是否为通路，每一个方向不通，则加一
    #接下来判断每个方向是否通路，不通则 back 加 1 
    ...
    if  Left(i,L,cv)==False:
        back += 1
    if back == 3: #如果back=3，就说明有三个方向都不通，说明走进了死路，需要掉头
        return True #需要掉头返回True

def zhuan(i,a,b,L,cv):        #使怪物转向：1.走进死路掉头，2：走到路口，随机转向（继续前进或掉头或转向）
                              # i是怪物的标签，a，b是现在的前进方向的参数，a为水平方向，向右为正，b是竖直方向，向下为正。
                              # 通过改变a,b的值并返回来改变前进方向
    if budongle(i,a,b,L,cv):  # 判读是否走进死路，如果是budongle会返回True 然后将前进方向掉头
        return -a,-b          # 将a，b变为相反数，即掉头
    else:
        if b == 0 :   #目前处于水平运动状态
            x=random()  #生成一个随机数
            if Up(i,L,cv) and Down(i,L,cv):   #上下都通，则走到十字路口，接下来向四个方向的前进各占四分之一
                if x > 0.75:    #1/4
                    a,b = 0,-1   #向上走
                elif x > 0.5:   #1/4
                    a,b = 0,1   #向下走
            ...
            elif Up(i,L,cv):   #水平运动时，走到向上通路的路口
                if x > 0.66:    #1/3
                    a,b = 0,-1   #向上走
                elif x > 0.33:  #1/3
                    a,b = -a,-b   #掉头
            ...
            elif Down(i,L,cv):   #水平运动时，走到向下通路的路口
                if x > 0.66:
                    a,b = 0,1   #向下走
                elif x > 0.33:
                    a,b = -a,-b   #向反方向走
            ...
            y=random()   #生成一个随机数
            if Right(i,L,cv) and Left(i,L,cv):   #上下运动时走到了十字路口，四个方向各四分之一的机率
                if y > 0.75:    #1/4
                    a,b = -1,0   #向左走
                elif y > 0.5:  #1/4
                    a,b = 1,0   #向右走
            ...
            elif Right(i,L,cv):   #上下运动时，走到了能够右转的路口                        
                if y > 0.66:
                    a,b = 1,0   #向右走
                elif y > 0.33:
                    a,b = -a,-b   #向反方向走
                    
            elif Left(i,L,cv):   #上下运动时，走到了能够左转的路口
                if y > 0.66:
                    a,b = -1,0   #向左走
                elif y > 0.33:
                    a,b = -a,-b   #向反方向走
...
def movetriangle(event):   #移动吃豆人  绑定键盘事件  
    global c    #c,d是吃豆人的移动时候的参数，c是水平方向的，向右为正 
    global d    #d是竖直方向的，向下为正
    if event.keysym == 'Up':  #如果按 ↑ 键，改变吃豆人的前进方向为向上 ，
        c,d=0,-1        
    elif event.keysym == 'Down':  #如果按 ↓ 键， 改变吃豆人的前进方向为向下 
        c,d=0,1         
    elif event.keysym == 'Left':   #如果按 ← 键， 改变吃豆人的前进方向为向左
        c,d = -1,0
    elif event.keysym == 'Right': #如果按 → 键， 改变吃豆人的前进方向为向右 
        c,d = 1,0


def deleteBeans(maps,cv,L):     #在第L关中的地图上除去与墙重叠的豆子。
                                #maps是储存了地图中的墙壁的矩形范围的坐标的txt文件
                                #txt文件中以每一个点的坐标为一行，两个点确定一个矩形
    itemsBesidesBeans=[13,20,20,17,30,13]   #每关中墙块、吃豆人和怪物的总数 
    f = open(maps, "r")              #读入地图中的矩形的大致坐标
    r = f.readlines()       #全部读取
    list1 = []       #创建用来存储坐标的list1
    for i in r:
        a = i.split()  #将r拆分为坐标数值的列表，以逗号的换行符为分隔
        for t in a:  #遍历列表
    ...
    listx1 = []     # 所以在这里将其分类到专门存储x1,x2,y1,y2的四个列表中
    listy1 = []
    listx2 = []
    listy2 = []
    for i in range(0, len(list1), 4): #每四个为一组 因为两个点确定一个矩形，所以四个一组确定一个矩形
    ...
    for i in range(len(listx1)):  #遍历坐标列表，四个一组为一个地图的矩形坐标
        ...
        x2 = listx2[i]
        y2 = listy2[i]
        list3 = list(cv.find_overlapping(x1, y1, x2, y2))  # 找出与地图重叠的豆豆，储存在list3中

        for t in list3: #遍历list3,
            if t > itemsBesidesBeans[L-1]:#如果t的图形项大于吃豆人怪物墙壁，那么t就是豆豆的图形项
                cv.delete(t)  # 删掉豆豆
...
def eat(cv,i,L,beans,windows):
    ...
    xya = list(cv.find_overlapping(xya1[0] - 10, xya1[1] - 10, xya1[0] + 10, xya1[
            1] + 10))       # 用find_overlapping找出与吃豆人矩形范围内重叠的其他图形（豆豆）的标识号，存到列表中
    for n in xya:           # 对于xya中的图形项
        if n > itemsBesidesBeans[L-1]: #如果是豆豆
            cv.delete(n)   #吃掉豆豆
            score+=10   #分数+10
...
def paintCanvas1(): #LEVEL1  第一关
    global askFail
    global score,life
    life = 3          #吃豆人的初始生命值
    score = 0           #初试分数
    win1 = Toplevel()   
    win1.resizable(0,0)  #固定窗口界面，使不能最大化窗口

    bg1 = Canvas(win1, width=600, height=600, bg="white")  # 画布创建
    bg1.pack()  #画布展开
    global a1

    p1 = (20, 20, 580, 20, 580, 580, 20, 580, 20, 20)  # 地图墙壁多边形的坐标
    ...
    w1_1 = bg1.create_polygon(pw1, outline="", fill="LightSkyBlue")  # 地图多边形的创建
    ...
    pic = PhotoImage(file="eater.gif")  # 吃豆人，eater.gif是吃豆人的图片
    a1 = bg1.create_image(97, 97, image=pic, anchor=CENTER)  # 初始放置吃豆人 

    dem = PhotoImage(file="demon.gif")  # 怪物，demon.gif是怪物的图片
    a10 = bg1.create_image(355,324, image=dem,anchor=CENTER)  #初始放置怪物
    ...
    for i in range(14):  # 将地图铺满豆豆
        for j in range(14):
            ...
            bi_j = bg1.create_oval(bij, outline="pink", fill="pink")#豆豆的形状：圆形 以及 颜色：粉色 
    ...
    bean=list(bg1.find_overlapping(0, 0, 600, 600))  #地图上所有的图形项，存储在列表bean
    bean1=bean[0:len(bean)]         #深拷贝bean
    ...
    c,d = 1,0         #c,d 是吃豆人的初始移动方向 
    x0,y0 = -1,0      #x,y 是怪物的初始移动方向
    x1,y1 = -1,0
    x2,y2 = -1,0
    x3,y3 = -1,0

    while True:
        bg1.bind_all('<KeyPress-Up>', movetriangle)   #绑定键盘事件与更改吃豆人移动参数的函数
        ...
        global ask
        itemsBesidesBeans=[13,20,20,17,30]   #墙块、吃豆人和怪物的总数（即不包括豆豆的最大图形项）
        final=list(bg1.find_overlapping(0,0,600,600)) #返回现在地图上所存在的所有图形项
        if final[-1] == itemsBesidesBeans[0] :   #如果最后一项是怪物的图形项，说明豆豆已经被吃完了
                                                #因为豆豆的图形项是比怪物墙壁吃豆人的大的，所以如果还有豆豆肯定在最后
                                                #所以如果最后一项不是豆豆，就说明豆豆吃完了
...
def paintCanvas2():#LEVEL2   #注释同第一关
    ...
    score = 0
    win2 = Toplevel()
    win2.resizable(0,0)  #固定窗口界面，使不能最大化窗口
    ...
    pw1 = (20, 20, 50, 20, 50, 550, 550, 550, 550, 20, 580, 20, 580, 580, 20, 580)  #地图墙壁的多边形的坐标
    ...
    ol2 = bg2.create_polygon(p2, outline='', fill="LightSkyBlue")       #地图墙壁的多边形的创建
...
def paintCanvas3():#LEVEL3   #注释同第一关
    ...
    score = 0
    win3 = Toplevel()
    win3.resizable(0,0)  #固定窗口界面，使不能最大化窗口
...
def paintCanvas4(): #LEVEL4   #注释同第一关
    ...
    score = 0
    win4 = Toplevel()
    win4.resizable(0,0)  #固定窗口界面，使不能最大化窗口
...
def paintCanvas5(): #LEVEL5   #注释同第一关
    ...
    score = 0
    win5 = Toplevel()
    win5.resizable(0,0)  #固定窗口界面，使不能最大化窗口
...
def Level_1(): #这是选择关卡窗口对应关卡的按钮所调用的函数，开始第一关
    select.destroy() #关闭选择关卡窗口
    paintCanvas1()   #开始这一关
...
def start():  #这是主窗口的开始游戏按钮调用的函数，由此函数进入第一关
    paintCanvas1()

def Invincible():  # 第一关无敌版   #内部注释同第一关
    winI = Toplevel()
    winI.resizable(0,0)  #固定窗口界面，使不能最大化窗口
    global cvI
    ...
    score = 1500000   #这是无敌版不同于正常关卡的地方，用于进行倒计时的变量score
    ...
    while True:
        ...
        xya = list(cvI.find_overlapping(xya1[0] - 10, xya1[1] - 10, xya1[0] + 10, xya1[
            1] + 10))  # 吃豆人的大小是20*20，所以(xya1[0]-10,xya1[1]-10,xya1[0]+10,xya1[1]+10)得到的是吃豆人矩形
                        #用find_overlapping找出与吃豆人矩形范围内重叠的其他图形（豆豆）的标识号，存到列表中
        ...
        for n in xya:  # 删掉xya中的图形项（豆豆），表示被吃掉了
            if n > 13:
                ...
                score -= 20000  #被怪物吃掉后，score减少20000

        sleep(0.0002)
        score -= 1   #表示每0.0002秒score会减少1（如果不被怪物吃掉，一般大约5分钟左右score变为0）
        ...
        if final[-1] == 13:   #这个胜利窗口可以告诉玩家无敌版的秘密
        ...
        if score <= 0:   #倒计时结束，玩家失败
            ...
            Label(askFailI, text="Your HP: %d" % (score), fg="black", font="Times -20 bold",
                  bg="white").grid(row=3, rowspan=2, sticky=W + E)  #此处提示玩家是有HP的（就是score的值）
...
def Destroy3():#用于退出关卡选择界面的命令函数
    select.destroy()

def SelectLevel(): #选择关卡，有对应关卡的按钮
    global select
    select = Toplevel()
    select.resizable(0,0)  #固定窗口界面，使不能最大化窗口
    # 以下是界面装饰部分
    ...
    #以下是关卡选择窗口中的标签和按钮，每个按钮对应每个关卡，以及退出该选择关卡窗口的按钮
    ...
    ExitButton3 = Button(select, text="exit", font="Times -20 bold", bg="black",
                         fg="white", command=Destroy3).place(x=437, y=700, anchor=CENTER)
...
def Help(): #帮助界面
    global rootwin1
    rootwin1 = Toplevel()
    rootwin1.resizable(0,0)  #固定窗口界面，使不能最大化窗口
    #以下是装饰部分
...
def Staff(): #制作人员界面
    global rootwin2
    rootwin2 = Toplevel()
    rootwin2.resizable(0,0)  #固定窗口界面，使不能最大化窗口
...
#以下为主界面及对应按钮
rootwin = Tk()
rootwin.resizable(0,0)   #固定窗口界面，使不能最大化窗口
